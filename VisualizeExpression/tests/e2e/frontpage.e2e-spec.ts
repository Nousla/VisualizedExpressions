...
describe('Frontpage', function () {
  ...
  it('should chose sandbox type and write an expression then generate a URL', function () {
    ...
    element(by.css('#frontpage_dropdownmenu')).click();
    var items = element.all(by.css('.ui-dropdown-item'));
    items.get(0).click();

    element(by.id('expressionInput')).sendKeys('4x+5=9').then(() => {
      return element(by.css('.frontpage_generatebtn')).click();
    }).then(() => {
      element(by.css('#frontpage_output_area')).getAttribute('value').then((value) => {
